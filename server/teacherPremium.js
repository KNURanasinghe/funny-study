...
const { executeQuery, generateId } = require('../server/setupDatabase');
...
router.get('/', async (req, res) => {
  try {
    ...
    const countQuery = 'SELECT COUNT(*) as total FROM findtutor_premium_teachers';
...
router.post('/', async (req, res) => {
  try {
    ...
    const params = [
      id, link_or_video, link1, link2, link3, video1, video2, video3, 
      ispaid, mail, paymentDate, stripeSessionId, paymentAmount
    ];
...
router.get('/status/:email', async (req, res) => {
  try {
    ...
    const query = 'SELECT * FROM findtutor_premium_teachers WHERE mail = ? LIMIT 1';
